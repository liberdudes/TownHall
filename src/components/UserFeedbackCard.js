...
class Report extends React.Component {
  render() {
    ...
    if (status === "New") {
      output = (
        <button id="new-badge" disabled={true}>
          New
        </button>
      );
    }
    if (status === "In Progress") {
      output = (
        <button id="in-progress-badge" disabled={true}>
          In Progress
        </button>
      );
    }
    if (status === "Completed") {
      output = (
        <button id="completed-badge" disabled={true}>
          Completed
        </button>
      );
    }
    if (status === "Closed") {
      output = (
        <button id="closed-badge" disabled={true}>
          Closed
        </button>
      );
    }
        ...
        <Container>
          <Row>
            <Col xs={9} id="col1-card">
              ...
              <p id="user-feedback-card-tag">{this.props.feedback.project}</p>
            </Col>
        ...
        <Container>
          <Row>
            ...
            <Col xs={4} id="col2-sub">
              <h5 id="user-votes">USER VOTES</h5>
              <p id="upvote-count">{this.props.feedback.upvote}</p>
            </Col>
          </Row>
        </Container>
        <Container>
          <Row>
            ...
            <Col xs={4}>
              <p id="user-feedback-card-date">{formattedDate}</p>
            </Col>
          </Row>
        </Container>
      </div>
